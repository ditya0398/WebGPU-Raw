...
//Define the position for the Geometry which will be passed to the Shader
...
class Renderer {
    ...
    declare positionCubeBuffer: GPUBuffer;

    declare positionPyramidBuffer: GPUBuffer;
    declare texturePyramidBuffer: GPUBuffer;
    ...
    declare viewParamsBufferCube: GPUBuffer;
    declare viewParamBGPyramid: GPUBindGroup;
    declare viewParamsBufferPyramid: GPUBuffer;

    //variable for textures
    declare textureCubebuffer: GPUBuffer;
    ...
    async initializeResources() {
        ...
        this.positionCubeBuffer = this.createBuffer(positionCube, GPUBufferUsage.VERTEX);
        this.textureCubebuffer = this.createBuffer(cubeTexCoords, GPUBufferUsage.VERTEX);
        this.positionPyramidBuffer = this.createBuffer(pyramidPositions, GPUBufferUsage.VERTEX);
        this.texturePyramidBuffer = this.createBuffer(pyramidTexCoords, GPUBufferUsage.VERTEX);
        ...
        let response = await fetch('./kundali.png');
        let imageBitmap = await createImageBitmap(await response.blob());

        const textureCube = this.device.createTexture({
        ...
        this.device.queue.copyExternalImageToTexture(
         ...
         response = await fetch('./stone.png');
         imageBitmap = await createImageBitmap(await response.blob());

        const texturePyramid = this.device.createTexture({
            size: [imageBitmap.width, imageBitmap.height, 1],
            format: 'rgba8unorm',
            usage:
                GPUTextureUsage.TEXTURE_BINDING |
        ...
        this.viewParamsBufferCube = this.device.createBuffer({
            size: 16 * 4,
        ...
        this.viewParamBGCube = this.device.createBindGroup({
            ...
            entries: [{ binding: 0, resource: { buffer: this.viewParamsBufferCube } },
            { binding: 1, resource: textureCube.createView() },
            { binding: 2, resource: this.sampler }]
        });


         //creating the uniform buffer
         this.viewParamsBufferPyramid = this.device.createBuffer({
            size: 16 * 4,
            usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
        });

        // Create a bind group which places our view params buffer at binding 0
        this.viewParamBGPyramid = this.device.createBindGroup({
            layout: bindGroupLayout,
            entries: [{ binding: 0, resource: { buffer: this.viewParamsBufferPyramid } },
            { binding: 1, resource: texturePyramid.createView() },
            { binding: 2, resource: this.sampler }]
        });
    ...
    encodeCommands() {
        ...
        mat4.translate(modelMatrix, modelMatrix, [2.0, 0.0, -5.0]);
        ...
        // this.commandEncoder.copyBufferToBuffer(upload, 0, this.viewParamsBufferCube, 0, 16 * 4);
        ...
        this.passEncoder.setVertexBuffer(0, this.positionCubeBuffer);

        this.passEncoder.setVertexBuffer(1, this.textureCubebuffer);
        ...
        mat4.translate(modelMatrix, modelMatrix, [-3.0, 0.0, -5.0]);
        ...
        this.device.queue.writeBuffer(
            this.viewParamsBufferPyramid,
            0,
            this.projView.buffer,
            this.projView.byteOffset,
            this.projView.byteLength
          );
          this.passEncoder.setVertexBuffer(0, this.positionPyramidBuffer);

          this.passEncoder.setVertexBuffer(1, this.texturePyramidBuffer);
          this.passEncoder.setBindGroup(0, this.viewParamBGPyramid);
